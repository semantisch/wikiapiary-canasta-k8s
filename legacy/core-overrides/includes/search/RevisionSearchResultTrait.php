		...
		if ( $title !== null ) {
			...
			( new HookRunner( $services->getHookContainer() ) )->onSearchResultInitFromTitle( $title, $id );
				...
				try {
					...
					// Legacy imported search rows can still point at titles that are
					// valid link targets but no longer materialize as proper pages in
					// MW 1.43. Treat them as missing revisions instead of fatalling.
